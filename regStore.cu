...
__device__ void mma_m16n8k16_f32_accum(
    ...
    float const &c1, float const &c2, float const &c3, float &c4
) {

    static_assert(std::is_same_v<value_t, half> || std::is_same_v<value_t, nv_bfloat16>, "value_t must half or nv_bfloat16");
...
__device__ void ldmatrix_x4_transpose(
    ...
    uint32_t smem_ptr = __cvta_generic_to_shared(load_from);
    asm volatile("ldmatrix.sync.aligned.x4.trans.m8n8.shared.b16" "{%0,%1,%2,%3};" 
    : 
    "=r"(a1), "=r"(a2), "=r"(a3), "=r"(a4)
    : 
    "r"(smem_ptr));
